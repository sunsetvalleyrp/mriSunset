...
local function processarRemocaoPeca(ped, veh, data, peca)
    ...
    -- Notifica o servidor para sincronizar com todos os outros players
    TriggerServerEvent('WLScripts:server:registrarPecaRemovida', data.netId, peca)

    Notify("Peça removida! Vá vender no ponto de venda.", "success")

    if data.quantidade_pecas_do_veiculo <= 0 then
        TriggerServerEvent('WLScripts:server:limparVeiculo', data.netId)
        DeleteVehicle(veh)
...
-- ============================================================
--  Thread: Detecta veículos nos locais de desmanche
-- ============================================================
...
-- ============================================================
--  Thread: Interação com as peças do veículo
-- ============================================================
...
-- ============================================================
--  Thread: Venda de peças nos pontos de venda
-- ============================================================
...
-- ============================================================
--  Sync: Recebe peça removida por outro player
-- ============================================================

RegisterNetEvent('WLScripts:client:syncPecaRemovida', function(vehNetId, peca)
    for veh, data in pairs(desmanchandoVeiculos) do
        if data.netId == vehNetId then
            -- Só processa se a peça ainda não foi marcada localmente
            if not data.pecasRemovidas[peca] then
                data.pecasRemovidas[peca] = true
                data.quantidade_pecas_do_veiculo = data.quantidade_pecas_do_veiculo - 1

                -- Aplica o efeito visual para este client também
                removerPecaDoVeiculo(veh, peca)
            end
            break
        end
