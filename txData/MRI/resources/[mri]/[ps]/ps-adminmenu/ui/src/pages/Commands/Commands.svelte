	...
	<div class="w-full h-[84%] flex flex-col gap-[1vh] mt-[1vh] overflow-auto">
		{#if $COMMANDS}
			{#if $COMMANDS && $COMMANDS.filter((commands) => commands.name.toLowerCase().includes(search.toLowerCase())).length === 0}
				<div class="text-tertiary text-center text-[1.7vh] font-medium mt-[1vh]">Nenhum comando encontrado.</div>
			{:else}
				<small class="font-medium">Total de Comandos: {SortedCommands.length}</small>
