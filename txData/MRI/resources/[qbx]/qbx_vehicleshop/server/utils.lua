...
function SpawnVehicle(src, data)
    ...
    -- Suporte a múltiplos pontos de spawn
    local spawnCoords
    if type(coords) == 'table' and #coords > 0 then
        for i = 1, #coords do
            ...
            for _, veh in ipairs(vehicles) do
                ...
                if #(vehCoords - vec3(point.x, point.y, point.z)) < 5.0 then
                    isClear = false
                    break
                end
            end
        ...
        -- Se todos estiverem ocupados, usa o primeiro mesmo assim
        if not spawnCoords then
            spawnCoords = coords[1]
        end
    else
        spawnCoords = coords
    end
